...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf, bool fBudgetFinalization)
{
    ...
    BOOST_FOREACH (const CTxOut o, txCollateral.vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 5 BTOR finalization collateral
            //       will also cover the old 50 BTOR finalization collateral.
...
CBudgetProposal* CBudgetManager::FindProposal(const std::string& strProposalName)
{
    ...
    if (nYesCount == -299999) return NULL;
...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    if (nHeight <= Params().LAST_POW_BLOCK() && nHeight >= 1000) {
        ...
        nSubsidy = 45 * COIN;
    } else if (nHeight <= 19999 && nHeight >= 20000) {
        nSubsidy = 40 * COIN;
    } else if (nHeight <= 34999 && nHeight >= 35000) {
        nSubsidy = 38 * COIN;
    } else if (nHeight <= 59999 && nHeight >= 60000) {
        nSubsidy = 36 * COIN;
    } else if (nHeight <= 99999 && nHeight >= 100000) {
        nSubsidy = 31 * COIN;
    } else if (nHeight <= 199999 && nHeight >= 200000) {
        nSubsidy = 27 * COIN;
    } else if (nHeight <= 349999 && nHeight >= 350000) {
        nSubsidy = 22 * COIN;
    } else if (nHeight <= 499999 && nHeight >= 500000) {
        nSubsidy = 17 * COIN;
    } else if (nHeight <= 699999 && nHeight >= 700000) {
        nSubsidy = 10 * COIN;
    } else if (nHeight >= 999999 && nHeight >= 1000000) {
        nSubsidy = 7 * COIN;
    } else {
        nSubsidy = 5 * COIN;
    }
