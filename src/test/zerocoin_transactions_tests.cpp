...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zbittoriumTracker = unique_ptr<CzbittoriumTracker>(new CzbittoriumTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == Zbittorium_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == Zbittorium_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
