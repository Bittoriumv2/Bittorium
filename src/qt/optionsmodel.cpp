...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BTOR);
    ...
    if (!settings.contains("nAnonymizeBittoriumAmount"))
        settings.setValue("nAnonymizeBittoriumAmount", 1000);

    nAnonymizeBittoriumAmount = settings.value("nAnonymizeBittoriumAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBittoriumAmount"))
        SoftSetArg("-anonymizebittoriumamount", settings.value("nAnonymizeBittoriumAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bittorium.cpp during shutdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBittoriumAmount:
            return QVariant(nAnonymizeBittoriumAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeBittoriumAmount:
            nAnonymizeBittoriumAmount = value.toInt();
            settings.setValue("nAnonymizeBittoriumAmount", nAnonymizeBittoriumAmount);
            emit anonymizeBittoriumAmountChanged(nAnonymizeBittoriumAmount);
            break;
        case CoinControlFeatures:
