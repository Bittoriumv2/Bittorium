...
#include "ui_zbtorcontroldialog.h"
...
std::set<std::string> ZBTORControlDialog::setSelectedMints;
std::set<CMintMeta> ZBTORControlDialog::setMints;

bool CZBTORControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZBTORControlDialog::COLUMN_DENOMINATION || column == ZBTORControlDialog::COLUMN_VERSION || column == ZBTORControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZBTORControlDialog::ZBTORControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZBTORControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZBTORControlDialog::~ZBTORControlDialog()
{
    delete ui;
}

void ZBTORControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBTORControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZBTORControlWidgetItem* itemDenom(new CZBTORControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZBTORControlWidgetItem *itemMint = new CZBTORControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zBTOR seed used to mint this zBTOR is not the same as currently hold in the wallet";
            else
...
void ZBTORControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBTORControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBTOR_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBTORControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBTORControlDialog::GetSelectedMints()
{
...
void ZBTORControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
