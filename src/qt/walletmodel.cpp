...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered btor address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BTOR.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal bittorium:URI (bittorium:XyZ...?message=example)
            {
...
static void NotifyZBittoriumReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyZBittoriumReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyZBittoriumReset.connect(boost::bind(NotifyZBittoriumReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyZBittoriumReset.disconnect(boost::bind(NotifyZBittoriumReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbittoriumTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
