...
#include "config/bittorium-config.h"
#endif
...
CZBittoriumWallet* zwalletMain = NULL;
...
void PrepareShutdown()
{
    ...
    RenameThread("bittorium-shutoff");
...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZBittoriumThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "bittorium.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "bittoriumd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the BTOR and zBTOR money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in BTOR/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in BTOR/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, bittorium, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in BTOR/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Bittorium specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-btorstake=<n>", strprintf(_("Enable or disable staking functionality for BTOR inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbtorstake=<n>", strprintf(_("Enable or disable staking functionality for zBTOR inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzbtor=<n>", strprintf(_("Enable automatic wallet backups triggered after each zBTOR minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbtorbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zBTOR backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizebittoriumamount=<n>     " + strprintf(_("Keep N BTOR anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (c) 2019-%i The Bittorium Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("bittorium-loadblk");
...
/** Sanity checks
 *  Ensure that Bittorium is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Bittorium Core is shutting down."));
...
#endif
    // Make sure only a single Bittorium process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bittorium Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Bittorium version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //Bittorium: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //bittorium specific: zerocoin and spork DB's
                ...
                // bittorium: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // bittorium: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Bittorium Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Bittorium Core to complete") << "\n";
        ...
        zwalletMain = new CZBittoriumWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zbittoriumWallet
        uiInterface.InitMessage(_("Syncing zBTOR wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZBittoriumBackups = GetBoolArg("-backupzbtor", true);
        pwalletMain->setZBittoriumAutoBackups(fEnableZBittoriumBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zbittoriumTracker->Init();
...
//    }
//
//    nAnonymizeBittoriumAmount = GetArg("-anonymizebittoriumamount", 0);
//    if (nAnonymizeBittoriumAmount > 999999) nAnonymizeBittoriumAmount = 999999;
//    if (nAnonymizeBittoriumAmount < 2) nAnonymizeBittoriumAmount = 2;
    ...
    LogPrintf("Anonymize bittorium Amount %d\n", nAnonymizeBittoriumAmount);
    ...
    /* Denominations
       ...
       For example:
       1BTOR+1000 == (.1BTOR+100)*10
       10BTOR+10000 == (1BTOR+1000)*10
    */
    ...
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZBittoriumThread(threadGroup);
    }
